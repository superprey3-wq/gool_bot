...
Rolling live pressure/momentum then validates whether OVER/UNDER agrees with the
actual match state instead of blindly following price movement.
"""
...
def _live_adjust(ctx,side):
 ...
 stats=ctx.get("stats") if isinstance(ctx.get("stats"),dict) else {};p=ctx.get("pressure") if isinstance(ctx.get("pressure"),dict) else {}
 ...
 # Static pace is a fallback when rolling pressure has not warmed up yet.
 pace=0.0
 if minute>0:pace=min(100.0,(xg/max(1,minute))*900+(sot/max(1,minute))*320+(big/max(1,minute))*500+(touches/max(1,minute))*28)
 hot=max(pressure,pace);diag={"pressure":round(pressure,1),"momentum":round(momentum,1),"quality":round(quality,1),"pace":round(pace,1),"xg":round(xg,2),"shots":round(shots,1),"sot":round(sot,1),"big":round(big,1),"touches_box":round(touches,1),"corners":round(corners,1)}
 if side=="UNDER":
...
def _candidate(key,rows,side,ctx):
 ...
 if reject:
  print(f"PROGRUZ_LIVE_REJECT event={eid} minute={minute} score={ctx.get('score')} pick={side}{line:g} reason={reject} pressure={diag['pressure']} momentum={diag['momentum']} pace={diag['pace']}",flush=True);return None
 ...
 odd=max(prices);sc=f"{int(ctx.get('home_score') or 0)}:{int(ctx.get('away_score') or 0)}";res={"id":"|".join((eid,"TOTAL",scope,line_txt,side)),"event_id":eid,"home":ctx.get("home") or "","away":ctx.get("away") or "","score_live":sc,"minute":minute,"status":ctx.get("status") or "LIVE","market":"TOTAL","scope":scope,"line":line,"side":side,"odd":odd,"books":conf,"moving_sources":sorted(sources),"median_delta_pct":round(-med,3),"persistent_books":persist,"strength":score,"live_stats":ctx.get("stats") or {},"live_pressure":diag,"live_adjustment":live_adj,"live_truth_source":"production_live_engine_flashscore","ts":time.time()}
 print(f"PROGRUZ_TRUTH_OK event={eid} score={sc} minute={minute} pick={side}{line:g} confirms={conf} move={med:.2f}% pressure={diag['pressure']} momentum={diag['momentum']} live_adj={live_adj:+d} strength={score:.0f}",flush=True);return res
def strong_rows():
...
class H(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  if p=="/bestbet":body=best_bet_payload()
   ...
   strong,source,meta,n,truth_events,err=strong_rows();body={"ok":True,"ts":time.time(),"mode":"LIVE_TOTAL_OU_FLASH_STATS_V7","market_source":source,"market_records":n,"truth_events":truth_events,"truth_error":err,"min_move_pct":MIN_MOVE,"live_stats_gate":True,"strong":strong}
...
if __name__=="__main__":print(f"GOOL_MARKET_SERVER FLASH STATS V7 port={PORT} live_truth_max_age={LIVE_MAX_AGE}s min_move={MIN_MOVE}% pressure_gate=on",flush=True);ThreadingHTTPServer((HOST,PORT),H).serve_forever()
