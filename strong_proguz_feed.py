"""GOOL Market Server: LIVE TOTAL O/U PROGRUZ with old BetB2B/1xBet confirmation."""
...
def _betb2b_confirmation(home,away,side):
 ...
 try:
  sig=betb2b.signal_for_match(home,away)
  ...
  return {"available":False,"confirmed":False,"error":f"{type(exc).__name__}:{exc}"}
...
def strong_rows():
 ...
 for key,rows in groups.items():
  ...
  med=statistics.median(pcts);best=min(pcts);r=rows[0]
  b2b=_betb2b_confirmation(r.get("home") or "",r.get("away") or "",key[4])
  base=55+min(18,len(books)*4)+min(15,abs(med)*4)+min(10,persist*3)
  if b2b.get("confirmed"):base+=6
  score=round(min(100,base),1)
  if score<MIN_SCORE:continue
  sources=["flashscore_lsapp"]
  if b2b.get("confirmed"):sources.append("betb2b_1xbet")
  out.append({"id":"|".join(key),"event_id":key[0],"home":r.get("home") or "","away":r.get("away") or "","score_live":r.get("score") or "","status":r.get("status") or "","market":"TOTAL","scope":key[2],"line":r.get("line"),"side":key[4],"odd":r.get("odd"),"books":len(books),"median_delta_pct":round(med,3),"best_delta_pct":round(best,3),"persistent_books":persist,"strength":score,"source":source,"confirmation_sources":sources,"betb2b":b2b,"ts":time.time()})
 out.sort(key=lambda x:(x["strength"],len(x.get("confirmation_sources") or []),x["books"],abs(x["median_delta_pct"])),reverse=True)
 return out[:20],source,meta,len(records),total_live

def _betb2b_loop():
 if not BETB2B_AVAILABLE:
  print(f"BETB2B_CONFIRM disabled import={globals().get('BETB2B_IMPORT_ERROR','unknown')}",flush=True);return
...
class H(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  if path=="/bestbet":body=best_bet_payload()
   ...
   common={"ok":True,"ts":time.time(),"mode":"LIVE_TOTAL_OU","market_source":source,"market_records":n,"live_total_rows":total_live,"betb2b_1xbet":BETB2B_AVAILABLE,"snapshot":meta}
   if path=="/health":body={**common,"db":str(DB),"best_bet_state_exists":BEST_STATE.exists()}
   elif path=="/markets":body=common
   else:body={**common,"strong":strong,"best_bet":best_bet_payload().get("signal")}
...
if __name__=="__main__":
 ...
 print(f"GOOL_MARKET_SERVER LIVE TOTAL O/U feed port={PORT} strong>={MIN_SCORE} betb2b_1xbet={BETB2B_AVAILABLE}",flush=True)
