...
def _source_name(r):
 # Count independent bookmaker identities, not just provider families. LSApp can
 # contain several different bookmakers and must not collapse them all to one.
...
def _candidate(key,rows):
 ...
 for x in rows:
  ...
  if p<=-0.6 and not bool(f.get("reversal")):moving.append((x,p))
 if not moving:return None
 sources={_source_name(x) for x,_ in moving};pcts=[p for _,p in moving];persist=sum(int(bool((x.get("flow") or {}).get("persistence"))) for x,_ in moving)
 # Use a row with a parseable fresh score whenever possible. Current price is the
 # best available decimal price among confirming moving books for this exact bet.
 score_rows=[x for x,_ in moving if _score_pair(x) is not None];anchor=(score_rows or [x for x,_ in moving])[0]
 pair=_score_pair(anchor);minute=_minute(anchor);med=statistics.median(pcts);best=min(pcts);b2b=_betb2b_confirmation(anchor.get("home") or "",anchor.get("away") or "",side,scope)
 ...
 # Exact live-total guard. For FT we require a real score and a line that is not
 # already settled. This blocks stale TM 3.5 at 2:3 and equally blocks stale TB
 # lines already exceeded. OVER and UNDER are otherwise evaluated symmetrically.
 if scope=="FULL_TIME":
  if pair is None:return None
  goals=sum(pair)
  if goals>=line:
   print(f"PROGRUZ_REJECT stale_ft_total event={key[0]} score={pair[0]}:{pair[1]} line={line:g} side={side}",flush=True);return None
 elif scope=="FIRST_HALF" and pair is not None and side=="UNDER" and sum(pair)>=line:return None
 score=58+min(16,confirmations*5)+min(14,abs(med)*3.0)+min(8,persist*2)+(5 if b2b.get("confirmed") else 0)+(2 if fast_exception else 0);score=round(min(100,score),1)
 ...
 odd,price_row=max(priced,key=lambda z:z[0])
 result={"id":"|".join(key),"event_id":key[0],"home":anchor.get("home") or "","away":anchor.get("away") or "","score_live":f"{pair[0]}:{pair[1]}" if pair else "","status":anchor.get("status") or "","minute":minute,"market":"TOTAL","scope":scope,"line":line,"side":side,"odd":odd,"odd_source":_source_name(price_row),"books":confirmations,"moving_sources":sorted(sources),"median_delta_pct":round(med,3),"best_delta_pct":round(best,3),"persistent_books":persist,"strength":score,"fast_exception":bool(fast_exception),"betb2b":b2b,"ts":time.time()}
 print(f"PROGRUZ_CANDIDATE event={key[0]} scope={scope} score={result['score_live']} minute={minute} pick={side} {line:g} odd={odd:.2f} books={confirmations} move={med:.2f}% strength={score:.0f}",flush=True);return result
def strong_rows():
...
class H(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  if path=="/bestbet":body=best_bet_payload()
   ...
   strong,source,meta,n,total_live=strong_rows();common={"ok":True,"ts":time.time(),"mode":"LIVE_PERIOD_TOTAL_OU_PRECISE_V2","market_source":source,"market_records":n,"live_total_rows":total_live,"min_move_pct":MIN_MOVE,"fast_min_move_pct":FAST_MIN_MOVE,"periods":["FULL_TIME","FIRST_HALF","SECOND_HALF"],"sides":["OVER","UNDER"],"betb2b_1xbet":BETB2B_AVAILABLE,"snapshot":meta}
...
if __name__=="__main__":
 threading.Thread(target=_betb2b_loop,name="betb2b-confirm",daemon=True).start();print(f"GOOL_MARKET_SERVER PRECISE TOTAL OVER+UNDER V2 port={PORT} strong>={MIN_SCORE} min_move={MIN_MOVE}% symmetric=on fresh_score_guard=on",flush=True);ThreadingHTTPServer((HOST,PORT),H).serve_forever()
