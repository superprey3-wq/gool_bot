...
def fetch_stats(event_id: str) -> str:
    headers={"User-Agent":UA,"x-fsign":FSIGN,"Origin":"https://www.flashscore.com","Referer":"https://www.flashscore.com/","Accept":"*/*","Accept-Language":"en-US,en;q=0.9"}
    for host in FEED_HOSTS:
        try:
            r=requests.get(f"https://{host}.flashscore.ninja/2/x/feed/df_st_1_{event_id}",headers=headers,timeout=12)
...
def parse_stats(body: str) -> dict[str, tuple[float,float]]:
    ...
    for chunk in body.split("~"):
        ...
        if m:
            sid,home,away=m.groups(); name=STAT_MAP.get(sid)
...
def calculate_goal_pressure(match,values,previous=None):
    ...
    quality=min(100.,xg*24+sot*5+big*10+inside*1.7+touches*.45)
    dxg=_delta(values,previous,"xg"); dshots=_delta(values,previous,"shots"); dsot=_delta(values,previous,"shots_on_target"); dbig=_delta(values,previous,"big_chances"); dinside=_delta(values,previous,"shots_inside_box"); dtouches=_delta(values,previous,"touches_box"); dcorners=_delta(values,previous,"corners")
    momentum=min(100.,dxg*42+dshots*7+dsot*13+dbig*18+dinside*4+dtouches*1.4+dcorners*5)
    goals=match.home_score+match.away_score
    context=65. if match.minute<=45 and goals<=1 else 45. if match.minute<=45 else 75. if match.minute<=75 and goals<=2 else 55. if match.minute<=75 else 70. if goals<=3 else 45.
    activity=min(100.,shots*2.2+sot*5.5+corners*1.8); score=min(100.,quality*.38+momentum*.37+activity*.15+context*.10)
    reasons=[]
...
async def discover_live_matches():
    ...
    async with async_playwright() as p:
        ...
        rows=page.locator("div[id*='g_1_']"); count=await rows.count(); logger.info("Flashscore rows for LIVE scan: %d",count)
        for i in range(count):
            ...
            text=" | ".join(lines); first=lines[0]; minute=0; is_halftime=bool(re.search(r"Half Time|HT|Перерыв",first,re.I)); mm=re.match(r"^(\d{1,2})(?:\+(\d+))?(?:'|\b)",first)
            ...
            if len(names)<2:
                candidates=[]
                for v in lines[1:]:
                    if re.fullmatch(r"\d+",v) or v in {"-","PREVIEW"}: continue
                    if re.search(r"[A-Za-zА-Яа-я]",v): candidates.append(v)
                    if len(candidates)==2: break
                names=candidates
            score_tokens=[int(x) for x in lines[1:] if re.fullmatch(r"\d+",x)]
            league=""
            try:
                # Competition header immediately preceding an event row is the most stable DOM source.
            ...
            if len(names)>=2 and len(score_tokens)>=2: matches.append(LiveMatch(event_id,minute,names[0],names[1],score_tokens[-2],score_tokens[-1],text[:180],league,is_halftime))
        await browser.close()
    logger.info("Validated LIVE matches: %d",len(matches)); return matches
...
def get_previous_values(event_id,current_minute,lookback_minutes=8):
    ...
    values=candidates[-1].get("values",{}); return {k:tuple(v) for k,v in values.items()}
