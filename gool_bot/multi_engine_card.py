...
def _status(market):
 s=str((market or {}).get('market_status') or (market or {}).get('market_consensus') or '').upper()
 return {'CONFIRMED':('✓ ПОДТВЕРЖДЕНО МОДЕЛЬЮ',GREEN),'DISAGREE':('⚠ РАСХОЖДЕНИЕ',RED),'CONFLICT':('⚠ РЫНОК ПРОТИВ',RED),'SINGLE_SOURCE':('1 ИСТОЧНИК',MUTED),'EARLY':('РАННИЙ РЫНОК',MUTED)}.get(s,(s or 'LIVE',MUTED))
...
def _header_text(engine):
 if engine=='first_half_goal':return PURPLE,'GOOL 2.0 • FIRST HALF GOAL','АНАЛИЗ С 1-Й МИНУТЫ','1T'
 return BLUE,'GOOL 2.0 • SECOND HALF OVER 1.5','РЕШЕНИЕ В ПЕРЕРЫВЕ','2T'
def _market_label(engine,market):
 if engine=='first_half_goal':
  try:return f"ТБ {float((market or {}).get('line')):g} В 1-М ТАЙМЕ"
  except:return 'ГОЛ В 1 ТАЙМЕ — ДА'
...
def render_engine_card(match,engine,score=0,delta=None,odd=None,result=None):
 ...
 panel(img,(55,510,1145,705),accent,22,2);d=ImageDraw.Draw(img);d.text((90,538),'GOOL 2.0 • КОНКРЕТНЫЙ ВХОД',font=F(19,True),fill=MUTED);d.text((90,574),'⭐ СТАВКА СТРАТЕГИИ',font=F(25,True),fill=accent);label=_market_label(engine,market);d.text((90,618),label,font=fit(d,label,635,35,True),fill=WHITE);d.text((90,662),f'КЭФ {float(odd):.2f}' if odd and float(odd)>1 else 'КЭФ НЕ НАЙДЕН',font=F(23,True),fill=GREEN if odd and float(odd)>1 else MUTED);d.text((790,545),'GOOL',font=F(17,True),fill=MUTED);d.text((790,582),f'{int(round(score))}/100',font=F(44,True),fill=accent);st,stc=_status(market);d.text((790,640),st,font=fit(d,st,310,18,True),fill=stc)
 d.text((70,742),'КОЭФФИЦИЕНТЫ • ПОДТВЕРЖДЕНИЕ',font=F(20,True),fill=accent);d.rounded_rectangle((55,777,1145,850),18,fill=(9,18,31),outline=LINE,width=1);d.text((80,798),_sources(market),font=fit(d,_sources(market),1040,22,True),fill=WHITE)
 ...
 d.text((70,1085),'ПОЧЕМУ ВХОД',font=F(21,True),fill=accent);why='Матч отслеживается с 0′. Вход появляется только в окне 15–25′, когда темп и качество моментов подтверждают гол до перерыва.' if engine=='first_half_goal' else 'Решение принимается только в перерыве по всему первому тайму: темп, xG/xGoT, створы и рынок ТБ1.5 второго тайма.'
...
def render_result_card(match,engine,result,market=None,odd=None):
 market=market or {};accent,title,_,mode=_header_text(engine);W,H=1200,840;img=Image.new('RGBA',(W,H),BG+(255,));d=ImageDraw.Draw(img);panel(img,(24,20,W-24,H-22),accent,32,3);d=ImageDraw.Draw(img);d.text((55,42),title,font=F(34,True),fill=WHITE);d.rounded_rectangle((1000,40,1140,95),15,outline=accent,width=2);d.text((1035,54),mode,font=F(20,True),fill=accent);is_win=str(result).lower()=='win';headline='WIN' if is_win else 'LOSE';sub='СТАВКА ЗАШЛА!' if is_win else 'СТАВКА НЕ ЗАШЛА';center(d,headline,112,F(72,True),GREEN if is_win else RED,W);center(d,sub,195,F(25,True),GREEN if is_win else RED,W);hn,an=logos(getattr(match,'event_id',''));crest(img,dl(hn),190,385,accent,165);crest(img,dl(an),1010,385,accent,165);panel(img,(425,292,775,477),accent,28,2);d=ImageDraw.Draw(img);center(d,f'{match.home_score} : {match.away_score}',330,F(72,True),WHITE,W);center(d,'HT' if engine=='first_half_goal' else 'FT',413,F(30,True),accent,W);label=_market_label(engine,market);center(d,label,545,fit(d,label,900,30,True),WHITE,W);price=float(odd or market.get('odd') or 0);center(d,f'КЭФ {price:.2f}' if price>1 else '',588,F(23,True),MUTED,W);center(d,'Результат рассчитан строго по ставке, указанной при входе',660,F(22),MUTED,W);center(d,'GOOL AI 2.0 • STRATEGY RESULT',775,F(19,True),MUTED,W);out=BytesIO();img.convert('RGB').save(out,'PNG',optimize=True);return out.getvalue()
