...
def render_engine_card(match,engine,score=0,delta=None,odd=None,result=None):
 ...
 center(d,getattr(match,"league","") or "LIVE FOOTBALL",458,F(21),MUTED,W);glow_round(img,(55,510,1145,690),accent,22,2);d=ImageDraw.Draw(img);d.text((90,538),"РЕШЕНИЕ СТРАТЕГИИ",font=F(18,True),fill=MUTED);d.text((90,572),"⭐ КОНКРЕТНАЯ СТАВКА",font=F(24,True),fill=accent);label=_market_label(engine,market);d.text((90,614),label,font=fit(d,label,610,34,True),fill=WHITE);d.text((90,655),f"КЭФ {float(odd):.2f}" if odd and float(odd)>1 else "КЭФ НЕ НАЙДЕН",font=F(23,True),fill=GREEN if odd and float(odd)>1 else MUTED);d.text((790,545),"РЕЙТИНГ",font=F(17,True),fill=MUTED);d.text((790,582),f"{int(round(score))}/100",font=F(44,True),fill=accent);d.text((790,637),_status(market) or "LIVE",font=fit(d,_status(market) or "LIVE",310,18,True),fill=GREEN)
 d.text((70,724),"РЫНОК • ПОДТВЕРЖДЕНИЕ",font=F(20,True),fill=accent);d.rounded_rectangle((55,760,1145,835),18,fill=(9,18,31),outline=LINE,width=1);d.text((80,781),_sources(market),font=fit(d,_sources(market),1040,22,True),fill=WHITE);labels=[("xG",_val(delta,"xg","float")),("xGoT",_val(delta,"xgot","float")),("УДАРЫ",_val(delta,"shots")),("В СТВОР",_val(delta,"shots_on_target")),("BIG CHANCES",_val(delta,"big_chances")),("В ШТРАФНОЙ",_val(delta,"shots_inside_box"))];y=865;gap=12;cw=(1090-gap*2)/3
 for i,(lab,val) in enumerate(labels):
  row=i//3;col=i%3;xa=55+col*(cw+gap);ya=y+row*98;_box(d,(xa,ya,xa+cw,ya+82),lab,val,WHITE)
 why_y=1070;d.text((70,why_y),"ПОЧЕМУ ВХОД",font=F(21,True),fill=accent);why="Бот собирает матч с 1-й минуты и допускает вход только в окне 15–25', если темп, качество моментов и рынок совпадают." if engine=="first_half_goal" else "Решение принимается только в перерыве по всему первому тайму: темп, xG/xGoT, створы и подтверждение рынка ТБ1.5 второго тайма."
