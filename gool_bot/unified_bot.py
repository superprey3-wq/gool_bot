...
LIVE_SIGNAL_THRESHOLD=float(os.getenv("LIVE_SIGNAL_THRESHOLD","80")); LIVE_GOAL_PROB_THRESHOLD=float(os.getenv("LIVE_GOAL_PROB_THRESHOLD","75")); LIVE_COOLDOWN_MINUTES=int(os.getenv("LIVE_COOLDOWN_MINUTES","12"))
...
def _entry_goal_probability(match,pressure,scope="FULL_TIME"):
    current=_scope_current_goals(match,scope); line=float(current)+0.5
    return _model_confidence(float(getattr(pressure,"score",0) or 0),float(getattr(pressure,"momentum",0) or 0),line,current,scope,int(getattr(match,"minute",0) or 0),None)
...
async def scan_live_once():
    ...
    for match in live:
        ...
        previous=get_previous_values(match.event_id,match.minute,8); pressure=calculate_goal_pressure(match,stats,previous); save_snapshot(match.event_id,StatsSnapshot(int(time.time()),match.minute,stats)); goal_times=parse_goal_timeline(fetch_summary(match.event_id)); now=time.time(); track_key=f"TRACK:{match.event_id}"; tracked=state.get(track_key); current_score=f"{match.home_score}:{match.away_score}"; goal_prob=_entry_goal_probability(match,pressure,"FULL_TIME")
        if not tracked:
            if pressure.score<LIVE_SIGNAL_THRESHOLD or goal_prob<LIVE_GOAL_PROB_THRESHOLD:
                logger.info("CORE_QUALITY_REJECT event=%s minute=%s master=%.1f/%s pgoal=%s/%s",match.event_id,match.minute,pressure.score,LIVE_SIGNAL_THRESHOLD,goal_prob,LIVE_GOAL_PROB_THRESHOLD);continue
