...
def _save_sent(data):
    cutoff=time.time()-21600; clean={}
    for k,v in data.items():
        if isinstance(v,dict):
            if float(v.get("ts",0))>=cutoff: clean[k]=v
        else:
            try: ts=float(v)
            except (TypeError,ValueError): continue
            if ts>=cutoff: clean[k]={"ts":ts}
...
def _model_confidence(pressure_score:float,momentum:float,line:float,current_goals:int,scope:str)->int:
    ...
    extra_needed=max(0.0,line-current_goals); base-=max(0.0,extra_needed-0.5)*13
    return max(45,min(94,round(base)))
def _recommendations(entries:list[dict[str,Any]],match,pressure):
    wanted_scope="SECOND_HALF" if match.is_halftime else "FULL_TIME"; current_goals=0 if wanted_scope=="SECOND_HALF" else match.home_score+match.away_score; recs=[]
    for entry in entries:
        if str(entry.get("bettingType"))!="OVER_UNDER" or str(entry.get("bettingScope") or "FULL_TIME")!=wanted_scope: continue
        for item in entry.get("odds") or []:
            ...
            recs.append({"scope":wanted_scope,"line":line,"odd":odd,"confidence":_model_confidence(pressure.score,pressure.momentum,line,current_goals,wanted_scope)})
    best_by_line={}
    for r in recs:
        if r["line"] not in best_by_line or r["odd"]>best_by_line[r["line"]]["odd"]: best_by_line[r["line"]]=r
    rows=list(best_by_line.values()); rows.sort(key=lambda r:(-r["confidence"],abs(r["odd"]-1.80))); useful=[r for r in rows if r["odd"]>=1.20]
    return (useful or rows)[:3]
def _format_signal(match,pressure,stats,recs,score_changed=False):
    def pair(key): a,b=stats.get(key,(0,0)); return f"{a:g} — {b:g}"
    reasons="\n".join(f"• {x}" for x in pressure.reasons[:4]) or "• устойчивое давление на ворота"; league_line=f"🏆 {match.league}\n" if match.league else "🏆 Турнир: данные уточняются\n"; status="Перерыв" if match.is_halftime else f"{match.minute}'"; title="🔵 <b>ПРОГНОЗ НА 2-Й ТАЙМ</b>" if match.is_halftime else "🔴 <b>LIVE-СИГНАЛ НА ГОЛ</b>"; changed="\n⚽ <b>Счёт изменился — прогноз пересчитан сразу.</b>\n" if score_changed else ""
    bet_lines=[]
    for i,r in enumerate(recs,1):
        scope_label="2-й тайм" if r["scope"]=="SECOND_HALF" else "матч"; bet_lines.append(f"{i}. <b>ТБ {r['line']:g} ({scope_label})</b> — кэф <b>{r['odd']:.2f}</b> | уверенность модели <b>{r['confidence']}%</b>")
    bets="\n".join(bet_lines) if bet_lines else "Рынки тоталов сейчас недоступны."
    return f"{title}\n\n⚽ <b>{match.home} — {match.away}</b>\n{league_line}⏱ {status} | Счёт <b>{match.home_score}:{match.away_score}</b>{changed}\n📊 <b>Статистика</b>\nОжидаемые голы (xG): <b>{pair('xg')}</b>\nУдары: {pair('shots')}\nУдары в створ: {pair('shots_on_target')}\nБольшие голевые моменты: {pair('big_chances')}\nУдары из штрафной: {pair('shots_inside_box')}\nКасания в штрафной: {pair('touches_box')}\nУгловые: {pair('corners')}\n\n⚡ Динамика давления: <b>{pressure.momentum:.0f}/100</b>\n🔥 Давление на гол: <b>{pressure.score:.0f}/100</b>\n\n🎯 <b>Рекомендованные варианты</b>\n{bets}\n\nПочему модель заинтересовалась матчем:\n{reasons}\n<i>Процент — оценка модели по текущей статистике, а не гарантированная вероятность.</i>"
async def scan_live_once():
    ...
    for match in live:
        ...
        previous=get_previous_values(match.event_id,match.minute,8); pressure=calculate_goal_pressure(match,stats,previous); save_snapshot(match.event_id,StatsSnapshot(int(time.time()),match.minute,stats)); logger.info("%s - %s %d' pressure=%.1f momentum=%.1f",match.home,match.away,match.minute,pressure.score,pressure.momentum)
        if pressure.score<LIVE_SIGNAL_THRESHOLD: continue
        now=time.time(); cooldown_key=f"{match.event_id}:HT" if match.is_halftime else match.event_id; old=sent_state.get(cooldown_key,{})
        if not isinstance(old,dict): old={"ts":float(old)}
        current_score=f"{match.home_score}:{match.away_score}"; previous_score=old.get("score"); score_changed=bool(previous_score and previous_score!=current_score)
        if not score_changed and now-float(old.get("ts",0))<LIVE_COOLDOWN_MINUTES*60: continue
        recs=_recommendations(_fetch_event_odds(match.event_id),match,pressure)
        if telegram_send(_format_signal(match,pressure,stats,recs,score_changed)):
            sent_state[cooldown_key]={"ts":now,"score":current_score,"minute":match.minute,"pressure":pressure.score}; _save_sent(sent_state); sent+=1
