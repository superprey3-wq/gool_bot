...
def _recommendations(entries:list[dict[str,Any]],match,pressure):
    ...
    rows=list(best.values()); rows.sort(key=lambda r:(-r["confidence"],abs(r["odd"]-1.8)))
    useful=[r for r in rows if r["odd"]>=1.20]
    return (useful or rows)[:3]

def _format_signal(match,pressure,stats,recs,reason="signal"):
    ...
    elif match.is_halftime: title="🔵 <b>ПРОГНОЗ НА 2-Й ТАЙМ</b>"
    elif reason=="followup": title="🔄 <b>ОБНОВЛЕНИЕ ПО МАТЧУ</b>"
    else: title="🔴 <b>LIVE-СИГНАЛ НА ГОЛ</b>"
    bet_lines=[]
    for i,r in enumerate(recs,1):
        label="2-й тайм" if r["scope"]=="SECOND_HALF" else "матч"
        bet_lines.append(f"{i}. <b>ТБ {r['line']:g} ({label})</b> — кэф <b>{r['odd']:.2f}</b> | уверенность модели <b>{r['confidence']}%</b>")
    bets="\n".join(bet_lines) if bet_lines else "Сейчас подходящего рынка тоталов нет."
...
async def scan_live_once():
    ...
    # Tracked matches disappear naturally when Flashscore no longer returns them as LIVE.
    ...
    for match in live:
        ...
        now=time.time(); track_key=f"TRACK:{match.event_id}"; tracked=state.get(track_key)
        ...
        if not tracked:
            ...
            if telegram_send(_format_signal(match,pressure,stats,recs,"signal")):
        ...
        last_ts=float(tracked.get("ts",0)); last_pressure=float(tracked.get("pressure",0)); pressure_jump=pressure.score>=LIVE_SIGNAL_THRESHOLD and pressure.score>=last_pressure+8
        regular_followup=pressure.score>=LIVE_SIGNAL_THRESHOLD and now-last_ts>=LIVE_COOLDOWN_MINUTES*60
        if score_changed or halftime_new or pressure_jump or regular_followup:
            reason="goal" if score_changed else "followup"
            recs=_recommendations(_fetch_event_odds(match.event_id),match,pressure)
            if telegram_send(_format_signal(match,pressure,stats,recs,reason)):
            ...
            # Continue following silently even when pressure temporarily falls.
