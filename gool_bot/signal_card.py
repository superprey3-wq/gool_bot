...
def render_signal_card(match:Any,pressure:Any,recs:list[dict[str,Any]]|None=None,kind='entry',master=None,probabilities=None)->bytes:
 win=kind=='goal';accent=GREEN if win else GOLD;H=820 if win else 1280;img=Image.new('RGBA',(W,H),BG+(255,));d=ImageDraw.Draw(img)
 ...
 if win:
  d.rounded_rectangle((70,515,1010,685),28,fill=(8,25,24),outline=GREEN,width=3)
  _center(d,'✓ ГОЛ ПОДТВЕРЖДЁН',558,_fit(d,'✓ ГОЛ ПОДТВЕРЖДЁН',850,44,True),GREEN)
  _center(d,'Сигнал GOOL сработал',620,_font(23,True),TEXT)
  footer=755
 else:
