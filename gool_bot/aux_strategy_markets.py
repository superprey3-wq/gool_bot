...
from bovada_live_odds import _find_event as bov_find, _over_prices as bov_over, get_second_half_over15
...
def second_half_over15(home:str,away:str)->list[dict[str,Any]]:
    """Strict standard second-half Over 1.5 from verified period markets only."""
    rows=[]
    try:
        r=get_second_half_over15(home,away)
        if r:rows.append(r)
    except Exception:pass
    try:
        e=kam_find(home,away)
        if e:
            ...
            for offer in data.get("betOffers") or []:
                ...
                for o in offer.get("outcomes") or []:
                    ...
                    if abs(line-1.5)<1e-9:rows.append({"scope":"SECOND_HALF","line":1.5,"odd":odd,"source":"Kambi/BetRivers"})
    except Exception:pass
    return [x for x in (_clean(r) for r in rows) if x]
...
def best_consensus(rows:list[dict[str,Any]])->dict[str,Any]|None:
    ...
    odds=[float(r["odd"]) for r in rows];best=dict(max(rows,key=lambda r:float(r["odd"])))
    best["source_prices"]=[{"source":r["source"],"odd":float(r["odd"])} for r in rows];best["source_count"]=len(rows)
    if len(rows)>=2:
