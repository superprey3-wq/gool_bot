...
No external ML runtime. Designed as an independent mathematical layer for GOOL BOT.
It consumes recent match-history rows already available in the project and returns
expected goals, 1X2, BTTS and over probabilities.
"""
...
def forecast_from_history(home_rows: list[Any], away_rows: list[Any], *, home_elo: float=1500.0, away_elo: float=1500.0) -> dict:
    """Rows may be objects or dicts exposing goals_for/goals_against or home/away goals.

    Missing history falls back conservatively to ~1.35 goals/team.
    """
    def gfga(row, side):
        if isinstance(row, dict):
            if 'goals_for' in row: return float(row.get('goals_for',0)), float(row.get('goals_against',0))
            h=float(row.get('home_goals', row.get('fh',0)) or 0); a=float(row.get('away_goals', row.get('fa',0)) or 0)
        else:
            if hasattr(row,'goals_for'): return float(row.goals_for), float(row.goals_against)
            h=float(getattr(row,'home_goals',0) or 0); a=float(getattr(row,'away_goals',0) or 0)
        return (h,a) if side=='home' else (a,h)

    h=[gfga(r,'home') for r in home_rows[-8:]]
    a=[gfga(r,'away') for r in away_rows[-8:]]
    hgf=_mean([x for x,_ in h],1.45); hga=_mean([y for _,y in h],1.20)
    agf=_mean([x for x,_ in a],1.20); aga=_mean([y for _,y in a],1.45)
    elo_h=_elo_expect(home_elo,away_elo)
    elo_a=1.0-elo_h

    # Blend attack/defence history with Elo; capped to avoid absurd friendly outputs.
    hx=0.56*((hgf+aga)/2)+0.44*(0.65+1.55*elo_h)
    ax=0.56*((agf+hga)/2)+0.44*(0.55+1.45*elo_a)
    hx=max(0.25,min(3.25,hx)); ax=max(0.20,min(3.00,ax))
