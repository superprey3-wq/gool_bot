"""Runtime for one-shot GOOL HT HUNTER and LATE RISK engines."""
...
def _save(d):
 cutoff=time.time()-10*3600;d={k:v for k,v in d.items() if float(v.get("ts",0) or 0)>=cutoff};STATE_FILE.write_text(json.dumps(d,ensure_ascii=False),"utf-8")
...
def _market(match,pressure,engine):
 ...
 return min(rows,key=lambda r:abs(float(r.get("odd",0))-2.2))
def _send_all(match,engine,score,d,odd,result=None):
 ...
 if result=="win":caption="✅ GOOL AI • ЗАШЁЛ! • "
 elif result=="loss":caption="❌ GOOL AI • НЕ ЗАШЁЛ • "
 else:caption="🔵 GOOL AI • HT HUNTER • " if engine==HT_HUNTER else "🔴 GOOL AI • LATE RISK • "
 caption+=("ПЕРВЫЙ ТАЙМ" if engine==HT_HUNTER else "ВТОРОЙ ТАЙМ")
 ok=0
 for cid in subs:
  try:
...
def _settle(engine,eid,result,match=None):
 key=_journal_key(engine,eid);fields={"result":"+" if result=="win" else "-","settled_ts":int(time.time())}
 if match:fields.update(final_score=f"{match.home_score}:{match.away_score}",goal_minute=int(match.minute))
 return update_signal(key,**fields)
...
def scan_engines(live):
 state=_load();live_by={str(m.event_id):m for m in live};active=_active_rows()
 # Settle active one-shot signals first.
 for row in active:
  eid=str(row.get("event_id"));engine=row.get("engine");m=live_by.get(eid)
  try:sh,sa=map(int,str(row.get("score_at_signal","0:0")).split(":"))
  except:sh=sa=0
  if m and int(m.home_score)+int(m.away_score)>sh+sa:
   st=state.get(f"{engine}:{eid}",{});_settle(engine,eid,"win",m);_send_all(m,engine,float(row.get("risk_score",0)),row.get("trend_delta") or st.get("delta") or {},row.get("odd"),"win");st["settled"]=True;state[f"{engine}:{eid}"]=st;continue
  if engine==HT_HUNTER and m and (getattr(m,"is_halftime",False) or int(m.minute)>45):
   _settle(engine,eid,"loss",m);_send_all(m,engine,float(row.get("risk_score",0)),row.get("trend_delta") or {},row.get("odd"),"loss")
  elif not m and time.time()-float(row.get("created_ts",0) or 0)>8*60:
   _settle(engine,eid,"loss",None)
 # Evaluate current live matches.
 for m in live:
  minute=int(getattr(m,"minute",0) or 0)
  if not (30<=minute<=45 or 70<=minute<=90):continue
  body=fetch_stats(m.event_id)
  if not body:continue
  stats=parse_stats(body)
  if not stats:continue
  key=str(m.event_id);s=state.setdefault(key,{"ts":time.time(),"snaps":[]});snaps=s.setdefault("snaps",[]);now_snap={"minute":minute,"stats":snapshot(stats)}
  if not snaps or int(snaps[-1].get("minute",-1))!=minute:snaps.append(now_snap)
  s["snaps"]=[x for x in snaps if minute-int(x.get("minute",minute))<=18][-20:];baseline=None
  old=[x for x in s["snaps"] if int(x.get("minute",0))<=minute-8]
  if old:baseline=old[-1].get("stats")
  if baseline is None:s["ts"]=time.time();continue
  d=delta(stats,baseline);goals=parse_goal_timeline(fetch_summary(m.event_id));last=_last_goal(goals);prev=get_previous_values(m.event_id,minute,8);pressure=calculate_goal_pressure(m,stats,prev)
  engines=[]
  ...
  for engine,dec in engines:
   ...
   if any(r.get("engine")==engine and str(r.get("event_id"))==str(m.event_id) for r in all_signals()):continue
   market=_market(m,pressure,engine)
   if not market:continue
   odd=float(market.get("odd",0) or 0)
   if _record(m,engine,dec.score,d,odd) and _send_all(m,engine,dec.score,d,odd):logger.info("ENGINE_SIGNAL %s %s %s minute=%d score=%.1f odd=%.2f",engine,m.home,m.away,minute,dec.score,odd)
  s["ts"]=time.time()
 _save(state)
