...
def metrics():
 ...
 for r in all_signals():
  ...
  g=_group(r);a=agg.setdefault(g,{'n':0,'wins':0,'losses':0,'pushes':0,'pnl':0.,'clv':[]});a['n']+=1;a[{'win':'wins','loss':'losses','push':'pushes'}[result]]+=1
 ...
 for a in agg.values():a['roi_pct']=round(a['pnl']/max(1,a['n'])*100,1);a['avg_clv_pp']=round(sum(a['clv'])/len(a['clv']),2) if a['clv'] else None
 _CACHE=(now,agg);return agg
def penalty_for(row):
 a=metrics().get(_group({'primary':row})) or {};n=int(a.get('n',0) or 0)
 if n<20:return 0.,'learning'
 roi=float(a.get('roi_pct',0) or 0);clv=a.get('avg_clv_pp');pen=0.
 if roi<-5:pen-=3
 if roi<-10:pen-=5
 if clv is not None and float(clv)<-1:pen-=3
 if n>=40 and roi<-12 and clv is not None and float(clv)<-2:return -25.,'auto_pause'
