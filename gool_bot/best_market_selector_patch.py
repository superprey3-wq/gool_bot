...
never be reused as probability for BTTS or a team total. Team totals are gated
by side-specific live evidence and by the number of additional goals required.
"""
...
def _goals_needed(row,m):
 ...
 # For standard x.5 team totals, win requires floor(line)+1 total team goals.
 return max(1,int(math.floor(line))+1-current)
def _team_evidence(row,m,p):
 side=_side(row);st=_stats(p)
 ...
 evidence=sum((xg>=.25,xgot>=.18,sot>=1,ibox>=2,touch>=5,big>=1,shots>=4))
 available=any(v>0 for v in (xg,xgot,sot,ibox,touch,big,shots))
 return {"xg":xg,"xgot":xgot,"sot":sot,"ibox":ibox,"touch":touch,"big":big,"shots":shots,"evidence":evidence,"available":available,"needed":_goals_needed(row,m)}
def _team_conf(row,m,p):
 ...
 # Extra required goals are exponentially harder; do not confuse P(next goal) with P(team total wins).
 need_penalty=max(0,ev["needed"]-1)*18
...
def _team_allowed(row,m,p):
 ...
 # No side-specific live evidence => team total cannot be CORE best bet.
 if not ev["available"]:return False,"NO_TEAM_STATS",ev
 # Requiring two or more future goals needs materially stronger side-specific evidence.
...
def _model_conf(row,m,p):
 try:
  ...
  # Never trust a generic upstream confidence for team totals/BTTS: recompute market-specific probability.
  ...
  if kind=="BTTS":
   ...
   # If one side still needs to score, require evidence from that exact side.
   if one:
    missing=0 if hs==0 else 1;st=_stats(p);xg=_pair(st,"xg")[missing];sot=_pair(st,"shots_on_target")[missing];shots=_pair(st,"shots")[missing]
    team_bonus=min(12.,xg*8+sot*2+shots*.5);return max(5.,min(88.,market*.42+pressure*.28+mom*.15+team_bonus))
...
def _rank(r,m,p):
 ...
 if kind.startswith("TEAM_TOTAL"):
  allowed,why,ev=_team_allowed(r,m,p)
  r["team_market_gate"]=why;r["team_goals_needed"]=ev["needed"];r["team_evidence"]=ev["evidence"]
  if not allowed:return -999.,{"selector_reject":why,"team_goals_needed":ev["needed"],"team_evidence":ev["evidence"]}
 conf=_model_conf(r,m,p);imp=_implied(odd);edge=conf-imp
 # Recompute edge from the selected market probability; do not inherit generic goal edge.
 r["value_edge"]=round(edge,1);sources=int(r.get("source_count") or r.get("bookmakers") or 1);movement=float(r.get("movement_score") or 0.0);score=conf*.58+max(-15.,min(20.,edge))*.65+_confirmation(r)+_price(odd)+min(6.,max(0,sources-1)*3.)+movement
...
def _market(entries,m,p):
 ...
 if ranked:
  ...
  market["best_concrete_bet"]={k:best.get(k) for k in ("scope","market_type","extra_market","team_side","team_name","line","selection","odd","source","source_prices","selector_score","selector_confidence","selector_implied","selector_edge","selector_movement","movement_status","movement_drop_pct","correlated_steam","market_status","team_goals_needed","team_evidence")}
  market["best_alternatives"]=[{k:r.get(k) for k in ("scope","market_type","team_name","line","selection","odd","source","selector_score","selector_confidence","selector_edge","selector_movement","movement_status","team_goals_needed","team_evidence")} for _,r in ranked[1:3]]
 return recs,market
lc._market=_market
