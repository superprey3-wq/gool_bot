...
1) FIRST_HALF_GOAL: one more goal in the first half, entry window 15'-25'.
2) SECOND_HALF_OVER15: Over 1.5 goals in the second half, decision at half-time.

Clock time can strengthen evidence but never create it. The retired risk engine
is kept only as an import compatibility alias; no risk strategy is evaluated.
"""
...
def first_half_goal(minute:int,d:Mapping[str,float],last_goal_minute:int|None=None,timing_bonus:float=0)->EngineDecision:
    ...
    if minute<FH_SIGNAL_FROM:return EngineDecision(FIRST_HALF_GOAL,False,score,"collecting first-half trend")
    ...
    # As the clock approaches 25', demand slightly stronger evidence rather than manufacturing a late signal.
    needed=3 if minute>=22 else 2
    return EngineDecision(FIRST_HALF_GOAL,score>=64 and evidence>=needed,score,f"fresh evidence={evidence}/{needed}; time_weight={time_weight(minute,FIRST_HALF_GOAL):.3f}; timing={float(timing_bonus or 0):+.1f}")

def second_half_over15(stats:Mapping[str,object],timing_bonus:float=0)->EngineDecision:
    """Half-time decision for standard 2H Over 1.5 using the full first half."""
...
def ht_hunter(minute:int,d:Mapping[str,float],last_goal_minute:int|None=None,timing_bonus:float=0)->EngineDecision:return first_half_goal(minute,d,last_goal_minute,timing_bonus)
def late_risk(minute:int,d:Mapping[str,float],last_goal_minute:int|None=None,timing_bonus:float=0)->EngineDecision:return EngineDecision(SECOND_HALF_OVER15,False,0.0,"retired compatibility alias")
