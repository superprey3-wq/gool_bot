...
def trend_score(d:Mapping[str,float])->float:
    raw=float(d.get("xg",0))*38+float(d.get("shots",0))*5+float(d.get("shots_on_target",0))*12+float(d.get("big_chances",0))*18+float(d.get("corners",0))*3+float(d.get("shots_inside_box",0))*3+float(d.get("touches_box",0))*.9
...
def ht_hunter(minute:int,d:Mapping[str,float],last_goal_minute:int|None=None,timing_bonus:float=0)->EngineDecision:
    minute=int(minute or 0);raw=trend_score(d);score=_adjust(raw,timing_bonus)
    ...
    return EngineDecision(HT_HUNTER,score>=68 and evidence>=2,score,f"fresh evidence={evidence}; timing={float(timing_bonus or 0):+.1f}")
def late_risk(minute:int,d:Mapping[str,float],last_goal_minute:int|None=None,timing_bonus:float=0)->EngineDecision:
    ...
    return EngineDecision(LATE_RISK,score>=78 and evidence>=3,score,f"fresh evidence={evidence}; timing={float(timing_bonus or 0):+.1f}")
