...
logger=logging.getLogger("fast_core_runtime")
MAX_STATS_WORKERS=12
CHEAP_PREFILTER=45.0
...
def _fetch_one(m):
    try:
        ...
        return m,parse_stats(body) if body else {}
    except Exception as exc:
        logger.info("FAST_CORE_STATS_FAILED %s: %s",getattr(m,"event_id","?"),exc)
        return m,{}


def _candidate_summary(m):
    try:
        body=fetch_summary(m.event_id)
        return parse_goal_timeline(body) if body else []
    except Exception as exc:
        logger.info("FAST_CORE_SUMMARY_FAILED %s: %s",getattr(m,"event_id","?"),exc)
        return []


async def scan_live_once_fast():
    live=await unified_bot.discover_live_matches()
    state=unified_bot._load_sent();now=time.time();sent=0
    ids={str(m.event_id) for m in live}
    ...
    for m in live:
        ...
        if f"TRACK:{m.event_id}" in state:continue
        pending.append(m)
    ...
    shortlisted=0;evaluated=0
    for m in pending:
        s=stats_by_id.get(str(m.event_id)) or {}
        if not s:continue
        prev=get_previous_values(m.event_id,m.minute,8)
        p=calculate_goal_pressure(m,s,prev)
        ...
        goals=_candidate_summary(m)
        recs=[];market={"available":False}
        qualifies,route,master,sc,hz,market=lc._evaluate(m,s,p,goals,market)
        evaluated+=1
        ...
        try:
            ...
            logger.info("FAST_CORE_ODDS_OPTIONAL_FAILED %s: %s",m.event_id,exc)
            recs=[];market={"available":False}
        ...
        state[f"TRACK:{m.event_id}"]={
            ...
            "route":route,"strategies":sc,"hazards":hz,"market":market,"window":lc._window_id(m.minute),
            "post_goal_pending":False,
        }
        sent+=1

    unified_bot._save_sent(state)
    logger.info("FAST_CORE_DIAG live=%d timed=%d shortlisted=%d evaluated=%d sent=%d tracked=%d",len(live),len(pending),shortlisted,evaluated,sent,sum(1 for k in state if str(k).startswith("TRACK:")))
    return sent
