...
# Production status is intentionally quiet: once per hour by default.
# Can still be overridden with GOOL_STATUS_INTERVAL_SECONDS, but never below 5 min.
STATUS_INTERVAL_SECONDS = max(300, int(os.getenv("GOOL_STATUS_INTERVAL_SECONDS", "3600")))
...
async def _scan_with_status():
    ...
    try:
        ...
        STATUS.update(online=online, analyzed=analyzed, tracked=sum(1 for key in state if str(key).startswith("TRACK:")), last_scan=time.time())
    return result
...
def send_heartbeat() -> bool:
    ...
    text = heartbeat_text();delivered = 0
    for chat_id in recipients:
        try:
            ...
            if r.ok:delivered += 1
            else:logger.warning("Heartbeat Telegram failed HTTP %s for %s",r.status_code,chat_id)
        except requests.RequestException as exc:logger.warning("Heartbeat Telegram failed for %s: %s",chat_id,exc)
    logger.info("GOOL heartbeat delivered %d/%d",delivered,len(recipients));return delivered > 0
