...
def _type(s):
 try:return int(s.get("type_id"))
  ...
  m=re.search(r"\bT(\d+)\b",str(s.get("market") or ""),re.I);return int(m.group(1)) if m else None
def _group(s):
 try:return int(s.get("group_id"))
 except Exception:
  m=re.search(r"\bG(\d+)\b",str(s.get("market") or ""),re.I);return int(m.group(1)) if m else None
def _total(s):return _type(s) in (9,10) and _group(s)==4 and s.get("last_line") is not None
def _name(s):
 ln=_num(s.get("last_line"),1).rstrip("0").rstrip(".");return ("ТБ " if _type(s)==9 else "ТМ ")+ln
def _sharp(s):
...
def _eligible_time(s):
 ...
 return bool(start and 0<=start-time.time()<=WINDOW)
def _time_label(s):
 try:
  minute=int(s.get("minute") or s.get("match_minute") or 0)
 ...
 return "LIVE / ближайшие 2 часа"
def _meaning(s,d):
 if _type(s)==9:return "🟢 рынок резко сильнее ждёт голы" if d>0 else "🔴 рынок резко уходит от голов"
...
def _message(s):
 ...
 parts=[
  "🚨 <b>ТОП-ПРОГРУЗ ТОТАЛА</b>",
  f"⚽ <b>{s.get('home') or '?'} — {s.get('away') or '?'}</b>",
  f"⏱ {_time_label(s)}",
  f"📊 <b>{_name(s)}</b>",
  "",
  "<b>БЫЛО → СТАЛО</b>",
  f"Кэф: <b>{_num(a)} → {_num(b)} {arrow}</b>",
 ]
 if p0 is not None and p1 is not None:parts.append(f"Вероятность: <b>{p0:.0f}% → {p1:.0f}%</b>")
 parts.extend([
  f"Изменение: <b>{d:+.2f} п.п.</b> за <b>{sec} сек</b>",
  "",
  f"{_meaning(s,d)}",
  "<i>Только тоталы матча ТБ/ТМ · LIVE или старт в ближайшие 2 часа · топ-5 · один сигнал на матч.</i>",
 ])
 return "\n".join(parts)
def _fetch():
...
def scan_once():
 global _PRIMED
 signals=[s for s in _fetch() if _total(s) and _sharp(s) and _eligible_time(s)]
 best={}
 for s in signals:
