...
def _live_signal_rows(rows):
    """Only actual entry decisions count as signals.

    goal = confirmation of an earlier entry, followup = service update. Neither is
    a new bet and therefore must never create its own win/loss row.
    """
...
def build_report_text()->str:
    ...
    for row in live_rows:
        ...
        if not body:live_wait+=1;continue
        try:fh,fa,_,_=_score_from_summary(body)
        except Exception:live_wait+=1;continue
        try:sh,sa=map(int,str(row.get("score_at_signal","0:0")).split(":"))
        except Exception:sh=sa=0
        hit=(fh+fa)>(sh+sa)
        if hit:live_ok+=1;mark="✅"
        elif event_id in current_live_ids:live_wait+=1;mark="⏳"
    ...
    for row in pre_rows:
        ...
        try:
            fh,fa,hh,ha=_score_from_summary(body);goals=_market_goals(str(row.get("scope","Матч")),fh,fa,hh,ha);res=_settle_total(str(row.get("side","ТБ")),float(row.get("line",0)),goals)
