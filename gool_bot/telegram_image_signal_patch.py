...
def _send(m,p,recs,text):
    ...
    if _send_photo_all(m,p,recs,kind,master):logger.info("TELEGRAM_IMAGE_SENT %s %d' %s — %s",kind,int(getattr(m,"minute",0) or 0),getattr(m,"home",""),getattr(m,"away",""));return True
    return False
