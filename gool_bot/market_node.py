"""GOOL lightweight secondary market node: Flashscore-gated + selected markets only."""
...
def _parse_fs_feed(text):
 ...
 for x in rows:
  ...
  out[x["AA"]]={"fs_id":x["AA"],"home":h,"away":a,"start_ts":ts,"status":x.get("AB","")}
 return out
def refresh_flashscore():
 ...
 try:
  ...
  LAST_FS=time.time();FS_ERROR="";log.info("FLASHSCORE_TODAY matches=%d",len(data));return len(data)
 except Exception as exc:
  LAST_FS=time.time();FS_ERROR=f"{type(exc).__name__}: {exc}";log.warning("FLASHSCORE_FAIL %s",FS_ERROR);return 0
def _fs_match(h,a,start_ts=0):
...
def _wanted_market(group,row,period):
 gid=group.get("G");tid=row.get("T");g=_norm(_txt(group,"GN","N","Name"));n=_norm(_txt(row,"N","Name","SN","SelectionName"));s=f"{g} {n}"
 if any(x in s for x in ("corner","card","booking","offside","throw in","shot","goal kick","exact score","correct score","next goal","penalty","substitution")):return False
 if any(x in s for x in ("handicap","spread","total","over","under","team total","individual total")):return True
 if any(x in s for x in ("1x2","match result","winner","result")):
  if any(x in n for x in ("draw"," x ","tie")):return False
  return True
 try:t=int(tid)
...
def _apply(now,h,a,eid,fs,main,markets):
 ...
 with LOCK:
  ...
  row.update({"home":h,"away":a,"event_id":eid,"fs_id":fs.get("fs_id"),"updated":now})
  if main:
   line,odd=main
...
def collect_once():
 ...
 try:
  ...
  LAST_ERROR="";LAST_CYCLE=now;CYCLES+=1;log.info("MARKET_NODE cycle=%d book_events=%d fs_today=%d matched=%d priced=%d selected_markets=%d state=%d",CYCLES,len(events),len(FS_MATCHES),matched,priced,selections,len(STATE))
 except Exception as exc:
...
def collect_prematch_once():
 ...
 try:
  ...
  for f in fsvals:
   ...
   if ts and datetime.fromtimestamp(ts,TZ).date()!=today:continue
   out[str(f["fs_id"])]={"event_id":f["fs_id"],"home":f["home"],"away":f["away"],"league":"","country":"","start_ts":ts,"start_local":datetime.fromtimestamp(ts,TZ).isoformat(timespec="minutes") if ts else "","source":"flashscore"}
  with LOCK:FIXTURES.clear();FIXTURES.update(out)
  LAST_PREMATCH=time.time();PREMATCH_ERROR="";log.info("PREMATCH_TODAY flashscore_fixtures=%d date=%s",len(out),today.isoformat());return len(out)
 except Exception as exc:
  PREMATCH_ERROR=f"{type(exc).__name__}: {exc}";LAST_PREMATCH=time.time();log.warning("PREMATCH_TODAY_FAIL %s",PREMATCH_ERROR);return 0
def _compact_events():return {k:{x:r.get(x) for x in ("home","away","event_id","fs_id","points","offered","missing_since","suspends","reopens","last_reopen_delta_pp","last_odds","updated")} for k,r in STATE.items()}
def _label(sel):
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  if self.path.startswith("/health"):
   with LOCK:mc=sum(len(r.get("markets") or {}) for r in STATE.values());fx=len(FIXTURES);fsn=len(FS_MATCHES)
   self._send(200,{"ok":True,"cycles":CYCLES,"last_cycle":LAST_CYCLE,"last_error":LAST_ERROR,"events":len(STATE),"markets":mc,"fixtures_today":fx,"flashscore_today":fsn,"last_flashscore":LAST_FS,"flashscore_error":FS_ERROR,"last_prematch":LAST_PREMATCH,"prematch_error":PREMATCH_ERROR,"flashscore_gate":True,"selected_markets_only":True});return
