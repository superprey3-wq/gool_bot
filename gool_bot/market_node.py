...
Collects BetB2B/1xBet live prices from a second IP. The legacy main-total stream
is kept for GOOL card lamps, while every priced selection returned by GetGameZip
is also tracked generically for the owner-only TEST market-anomaly engine.
"""
...
def _all_markets(event):
 """Return every priced selection we can identify, without guessing market semantics."""
 out=[]
 period=_txt(event,"PN","PeriodName") or str(event.get("P") or "FT")
 for group in event.get("GE") or []:
  ...
  for bucket_i,bucket in enumerate(group.get("E") or []):
   for row_i,row in enumerate(bucket or []):
    ...
    name=_txt(row,"N","Name","SN","SelectionName")
    # Include identifiers that distinguish selections sharing G/T/P.
    sel_id=row.get("I") or row.get("ID") or row.get("E") or row.get("PL") or f"{bucket_i}:{row_i}"
...
def _apply(now,h,a,eid,main_market,markets):
 ...
 with LOCK:
  ...
  for item in markets:
   mk=item["key"];seen.add(mk)
   sel=book.setdefault(mk,{"points":[],"offered":False,"missing_since":None,"suspends":0,"reopens":0,"last_reopen_delta_pp":0.0,"last_odds":None})
   _update_selection(sel,now,item)
  # A disappearance is only a suspend candidate after a successful full event payload.
  ...
  # Keep memory bounded.
  stale=[mk for mk,sel in book.items() if float(sel.get("updated",0) or 0)<now-21600]
  for mk in stale:book.pop(mk,None)
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  if self.path.startswith("/health"):
   with LOCK:market_count=sum(len((r.get("markets") or {})) for r in STATE.values())
   self._send(200,{"ok":True,"cycles":CYCLES,"last_cycle":LAST_CYCLE,"last_error":LAST_ERROR,"events":len(STATE),"markets":market_count});return
...
def main():
 threading.Thread(target=collector,daemon=True).start();srv=ThreadingHTTPServer(("0.0.0.0",PORT),Handler);log.info("MARKET_NODE_HTTP port=%d interval=%ds max_events=%d all_markets=%d",PORT,INTERVAL,MAX_EVENTS,MAX_MARKETS_PER_EVENT);srv.serve_forever()
