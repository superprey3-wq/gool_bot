...
def auditable_primary(primary:dict[str,Any]|None)->bool:
    ...
    try:return bool(primary.get("scope")) and float(primary["line"])>=0 and float(primary["odd"])>1.0
    except (KeyError,TypeError,ValueError):return False

def required_edge(reason:str)->float:
    reason=str(reason or "signal")
    if reason=="reentry":return 7.0
    if reason=="first_half_goal":return 6.0
    if reason=="second_half_over15":return 6.0
    return 5.0
...
def value_ok(primary:dict[str,Any]|None,reason:str)->tuple[bool,str]:
    ...
    if edge<minimum:return False,f"edge={edge:.1f}<{minimum:.1f}"
    return True,"ok"
