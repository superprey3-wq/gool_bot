...
COUNTRY_RU = {
    "Bul": "Болгария", "Gre": "Греция", "Nor": "Норвегия", "Bel": "Бельгия", "Aze": "Азербайджан",
    "Den": "Дания", "Eng": "Англия", "Esp": "Испания", "Ita": "Италия", "Ger": "Германия", "Fra": "Франция",
}
KNOWN_RU = {
    "CSKA": "ЦСКА", "Sofia": "София", "Panathinaikos": "Панатинаикос", "Bodo/Glimt": "Буде/Глимт",
    "Royale Union SG": "Юнион Сент-Жиллуаз", "Sabah Baku": "Сабах Баку", "Aarhus": "Орхус",
}
...
def _ru_name(name: str) -> str:
    """Readable Russian label for Telegram; preserve numbers and common club abbreviations."""
    text=name.strip()
    country=""
    m=re.search(r"\s*\(([A-Za-z]{3})\)\s*$", text)
    if m:
        country=COUNTRY_RU.get(m.group(1),m.group(1)); text=text[:m.start()].strip()
    for src,dst in sorted(KNOWN_RU.items(), key=lambda kv:-len(kv[0])):
        text=text.replace(src,dst)
    # Lightweight transliteration fallback for still-Latin club names.
    ...
    def translit_word(w:str)->str:
        ...
        table=str.maketrans("abvgdezijklmnoprstufhycqxw", "абвгдезийклмнопрстуфхыцкв")
        low=low.translate(table)
...
def _ru_league(value:str)->str:
    ...
    replacements={"Champions League":"Лига чемпионов","Europa League":"Лига Европы","Conference League":"Лига конференций","Friendly":"Товарищеский матч","Club Friendly":"Клубный товарищеский матч"}
    out=value
...
def _parse_feed(text:str)->list[Match]:
    ...
    for raw in text.split("~AA¬")[1:]:
        ...
        league=(fields.get("ZA","") or fields.get("CX","") or fields.get("ZB","")).strip()
...
async def _discover_from_browser()->list[Match]:
    ...
    async with async_playwright() as p:
        ...
        context=await browser.new_context(user_agent=HEADERS["User-Agent"],locale="ru-RU",timezone_id="UTC",viewport={"width":1440,"height":1200})
...
def _discover_matches()->list[Match]:
    ...
    # Browser metadata is richer (tournament + localized labels); merge it when available.
    try:
        browser=asyncio.run(_discover_from_browser())
...
def _extract_signals(entries,match):
    ...
    for entry in entries:
        ...
        for item in items:
            ...
            pid=item.get("eventParticipantId"); market="Общий тотал" if pid is None else f"ИТ {_ru_name(participant_names.get(pid,'команды'))}"; side="ТБ" if selection=="OVER" else "ТМ"
...
def _format(match,signals):
    start=match.kickoff.astimezone(MOSCOW).strftime("%H:%M")
    lines=["🔥 <b>ПРЕДМАТЧЕВЫЙ ПРОГРУЗ</b>","",f"⚽ <b>{_ru_name(match.home)} — {_ru_name(match.away)}</b>",f"🏆 {_ru_league(match.league)}",f"🕒 Старт: <b>{start} МСК</b>",""]
    for s in signals:
        lines += [f"<b>{s.scope} · {s.market} {s.line} · {s.side}</b>",f"📉 {s.median_open:.2f} → {s.median_current:.2f} (-{s.median_drop:.1f}%)",f"🏦 Синхронно: {s.consensus*100:.0f}% ({s.bookmakers} букмекеров)",f"🔥 Давление рынка: {s.score:.1f}/10",""]
    lines.append("<i>Источник: Flashscore/LSApp · открытие линии → текущий коэффициент</i>")
    return "\n".join(lines)
def main():
    sent=0
    for match in _discover_matches():
        ...
        if signals and _telegram_send(_format(match,signals)): sent+=1
