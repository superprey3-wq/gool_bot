...
def _build_probe(matches):
    ...
    for m in matches:
        ...
        if not event:lines.append(f"❌ не найдено · match {round(float(sim)*100)}%");continue
        ...
        if not game:lines.append(f"⚠️ GetGameZip: {game_err or 'пустой ответ'}");continue
...
def _handle_message(message:dict):
    ...
    tg._send_reply(chat_id,'🧪 Проверяю 1xBet: гол в 1-м тайме / тоталы матча / обе забьют…')
    try:
