...
import telegram_subscribers,subscriber_persistence_patch,telegram_interactive_live_patch,owner_market_tape_patch,market_test_signal,market_recommendation_results,market_results_telegram_patch
...
async def market_test_loop():
 while True:
  try:
   await asyncio.sleep(3);_mem("market_test_before");sent=await asyncio.to_thread(market_test_signal.scan_once);_mem("market_test_after")
...
async def main():
 ...
 logger.info("GOOL LIVE | build=%s | live-only primary | remote_market_node=%s | market recommendations tracked",BUILD_ID,bool(market_node_bridge.URL));_mem("main_started")
 try:
  while True:
