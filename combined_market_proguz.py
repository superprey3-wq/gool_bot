"""MonkeyBytes combined runtime: live + prematch + PROGRUZ + BEST BET + read-only feed."""
from __future__ import annotations
import os, signal, subprocess, sys, time, urllib.request
from pathlib import Path
HOME=Path(os.getenv("GOOL_HOME","/home/container")); PROGUZ_DIR=HOME/"proguz_runtime"; BESTBET_DIR=HOME/"bestbet_runtime"
REPO="https://github.com/superprey3-wq/gool_bot.git"; PROGUZ_BRANCH="live-only-quant-foundation"; BESTBET_BRANCH="main"
...
def sync_proguz():sync_repo(PROGUZ_DIR,PROGUZ_BRANCH)
...
def install_requirements():
 seen=set()
 for req in (HOME/"requirements-browser-node.txt",PROGUZ_DIR/"requirements.txt",PROGUZ_DIR/"gool_bot"/"requirements.txt",BESTBET_DIR/"requirements.txt",BESTBET_DIR/"gool_bot"/"requirements.txt"):
  if req.exists() and str(req.resolve()) not in seen: seen.add(str(req.resolve())); run([sys.executable,"-m","pip","install","--user","-r",str(req)])
def child_env():
 env=os.environ.copy(); env.setdefault("PYTHONUNBUFFERED","1"); env.setdefault("GOOL_MARKET_STATE",str(HOME/"market_node_state.json")); env.setdefault("GOOL_MARKET_HISTORY",str(HOME/"market_node_history_v6.json")); env.setdefault("GOOL_PREMATCH_STATE",str(HOME/"prematch_market_state.json")); env.setdefault("GOOL_PREMATCH_POLL_SECONDS","300"); env.setdefault("GOOL_PREMATCH_MAX_FIXTURES","220"); env.setdefault("GOOL_PREMATCH_ODDS_EVENTS","100"); env.setdefault("GOOL_STRONG_MIN_SCORE","80"); env.setdefault("GOOL_REMOTE_BEST_BET_STATE",str(HOME/"remote_best_bet_state.json")); env.setdefault("GOOL_REMOTE_BEST_BET_POLL_SECONDS","60"); env.setdefault("SIGNAL_JOURNAL_FILE",str(HOME/"remote_best_bet_journal.json")); return env
...
def main():
 print("GOOL COMBINED LIVE+PREMATCH+PROGRUZ+BEST_BET+FEED starting",flush=True); sync_proguz(); sync_bestbet(); sync_asset("prematch_market_node.py",PREMATCH); sync_asset("strong_proguz_feed.py",FEED); install_requirements(); env=child_env(); live=start(COLLECTOR,env); prematch=start(PREMATCH,env); proguz=start_proguz(env); bestbet=start_bestbet(env); feed=start(FEED,env); print(f"GOOL COMBINED ONLINE live_pid={live.pid} prematch_pid={prematch.pid} proguz_pid={proguz.pid} bestbet_pid={bestbet.pid} feed_pid={feed.pid} strong>=80",flush=True)
 stopping=False
 def sig(*_):
 ...
 try:
  while not stopping:
   ...
   if bestbet.poll() is not None:print(f"GOOL COMBINED bestbet exited rc={bestbet.returncode}; restarting",flush=True);time.sleep(3);bestbet=start_bestbet(env)
   ...
   time.sleep(3)
 finally:stop(feed);stop(bestbet);stop(proguz);stop(prematch);stop(live);print("GOOL COMBINED stopped",flush=True)
