...
def main():
 print("GOOL MONKEY unified Flashscore truth + PROGRUZ V12 cumulative intelligence + BEST BET",flush=True);sync_repo(BESTBET_DIR,BESTBET_BRANCH)
 for n,p in (("browser_market_node.py",COLLECTOR),("browser_market_all.py",LIVE),("strong_proguz_feed.py",FEED_BASE),("strong_proguz_v9.py",FEED_V11),("strong_proguz_v12.py",FEED),("proguz_market_flow.py",FLOW),("proguz_fair_probability.py",FAIR),("market_store_bridge.py",BRIDGE),("monkey_live_context.py",CONTEXT)):sync_asset(n,p)
 sync_asset("betb2b_market_signal.py",BETB2B,OLD_RAW_BASE);e=env();spec={"context":(CONTEXT,HOME),"live":(LIVE,HOME),"feed":(FEED,HOME),"store":(BRIDGE,HOME),"bestbet":(BESTBET_DIR/"gool_bot"/"best_bet_remote_worker.py",BESTBET_DIR/"gool_bot")};procs={name:start(script,e,cwd) for name,(script,cwd) in spec.items()};print("GOOL MONKEY ONLINE flashscore_truth=on live_stats=on proguz_v12=on instant_steam=on cumulative_steam=on devig=on lead_lag=on audit=on bestbet=on",flush=True)
 stopping=False
 def sig(*_):
