...
COLLECTOR=HOME/"browser_market_node.py"; PREMATCH=HOME/"prematch_market_node.py"
PREMATCH_RAW="https://raw.githubusercontent.com/superprey3-wq/gool_bot/browser-market-node/prematch_market_node.py"
...
def sync_prematch():
 try:
  data=urllib.request.urlopen(PREMATCH_RAW,timeout=20).read(); PREMATCH.write_bytes(data); print(f"GOOL PREMATCH asset synced bytes={len(data)}",flush=True)
 except Exception as e:
  print(f"GOOL PREMATCH asset sync failed: {e}",flush=True)
  if not PREMATCH.exists(): raise
...
def main():
 print("GOOL COMBINED LIVE+PREMATCH+PROGRUZ starting",flush=True); sync_proguz(); sync_prematch(); install_requirements(); env=child_env(); live=start(COLLECTOR,env); prematch=start(PREMATCH,env); proguz=start_proguz(env); print(f"GOOL COMBINED ONLINE live_pid={live.pid} prematch_pid={prematch.pid} proguz_pid={proguz.pid} schedule=today+tomorrow odds=on",flush=True)
 stopping=False
 def sig(*_):
 ...
 try:
  while not stopping:
   ...
   time.sleep(3)
 finally: stop(proguz); stop(prematch); stop(live); print("GOOL COMBINED stopped",flush=True)
