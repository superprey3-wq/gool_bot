...
LIVE=HOME/"browser_market_all.py";COLLECTOR=HOME/"browser_market_node.py";FEED=HOME/"strong_proguz_feed.py";BRIDGE=HOME/"market_store_bridge.py";BETB2B=HOME/"betb2b_market_signal.py";CONTEXT=HOME/"monkey_live_context.py"
...
def main():
 print("GOOL MONKEY unified Flashscore truth + PROGRUZ + BEST BET",flush=True);sync_repo(BESTBET_DIR,BESTBET_BRANCH)
 for n,p in (("browser_market_node.py",COLLECTOR),("browser_market_all.py",LIVE),("strong_proguz_feed.py",FEED),("market_store_bridge.py",BRIDGE),("monkey_live_context.py",CONTEXT)):sync_asset(n,p)
 sync_asset("betb2b_market_signal.py",BETB2B,OLD_RAW_BASE);e=env();spec={"context":(CONTEXT,HOME),"live":(LIVE,HOME),"feed":(FEED,HOME),"store":(BRIDGE,HOME),"bestbet":(BESTBET_DIR/"gool_bot"/"best_bet_remote_worker.py",BESTBET_DIR/"gool_bot")};procs={name:start(script,e,cwd) for name,(script,cwd) in spec.items()};print("GOOL MONKEY ONLINE flashscore_truth=on live_stats=on proguz=on bestbet=on",flush=True)
 stopping=False
 def sig(*_):
