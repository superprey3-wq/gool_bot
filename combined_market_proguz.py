"""MonkeyBytes runtime: prematch/live market history + standalone live truth + Strong PROGRUZ only."""
from __future__ import annotations
import os,signal,subprocess,sys,time,urllib.request
from pathlib import Path

HOME=Path(os.getenv("GOOL_HOME","/home/container"));LIVE_RT=HOME/"proguz_live_runtime"
...
RAW_BASE="https://raw.githubusercontent.com/superprey3-wq/gool_bot/browser-market-node/";MAIN_RAW="https://raw.githubusercontent.com/superprey3-wq/gool_bot/main/gool_bot/";OLD_RAW_BASE="https://raw.githubusercontent.com/superprey3-wq/gool_bot/live-only-quant-foundation/gool_bot/"

def sync_asset(name,path,base=RAW_BASE):
 path.parent.mkdir(parents=True,exist_ok=True);data=urllib.request.urlopen(base+name,timeout=20).read();path.write_bytes(data);print(f"GOOL asset {name} bytes={len(data)}",flush=True)
...
def env():
 ...
 e.setdefault("GOOL_MONKEY_LIVE_CONTEXT",str(HOME/"monkey_live_context.json"));e.setdefault("GOOL_MONKEY_LIVE_POLL_SECONDS","20");e.setdefault("GOOL_PROGRUZ_LIVE_RUNTIME",str(LIVE_RT));e.setdefault("LIVE_STATE_FILE",str(HOME/"monkey_live_stats_history.json"))
...
def main():
 print("GOOL MONKEY MARKET-ONLY | PREMATCH + LIVE ODDS + LIVE TRUTH + STRONG PROGRUZ V12 | MAIN=OFF | TELEGRAM=OFF | BEST BET=OFF",flush=True)
 # Market/progruz assets.
 ...
 # Only the two lightweight production files Strong Proguz needs for Flashscore truth.
 sync_asset("live_engine.py",LIVE_RT/"live_engine.py",MAIN_RAW);sync_asset("feed_live_discovery.py",LIVE_RT/"feed_live_discovery.py",MAIN_RAW)
 e=env();spec={"context":(CONTEXT,HOME),"prematch":(PREMATCH,HOME),"live":(LIVE,HOME),"feed":(FEED,HOME),"store":(BRIDGE,HOME)}
 procs={name:start(script,e,cwd) for name,(script,cwd) in spec.items()}
 print("GOOL MONKEY ONLINE mode=market_only processes=context,prematch,live,feed,store main_v3=off telegram=off journal=off reports=off bestbet=off strong_proguz=on prematch=on live_market=on",flush=True)
 stopping=False
 def sig(*_):
