"""MonkeyBytes runtime: pre-match + live market history and strong PROGRUZ only."""
...
def main():
 print("GOOL MONKEY PREMATCH->KICKOFF->LIVE + STRONG PROGRUZ V12 | BEST BET=OFF",flush=True)
 ...
 sync_asset("betb2b_market_signal.py",BETB2B,OLD_RAW_BASE);e=env();spec={"context":(CONTEXT,HOME),"prematch":(PREMATCH,HOME),"live":(LIVE,HOME),"feed":(FEED,HOME),"store":(BRIDGE,HOME)};procs={name:start(script,e,cwd) for name,(script,cwd) in spec.items()};print("GOOL MONKEY ONLINE prematch=on horizon=6h continuous_market_history=on flashscore_truth=on live_stats=on proguz_v12=on instant_steam=on cumulative_steam=on devig=on lead_lag=on audit=on bestbet=off",flush=True)
 stopping=False
 def sig(*_):
