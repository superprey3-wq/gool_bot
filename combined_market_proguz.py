"""MonkeyBytes runtime: lightweight LIVE TOTAL O/U PROGRUZ + remote BEST BET.

The Flashscore/LSApp JSON state is the critical path. Optional persistence and
BEST BET cannot stop the live collector/feed. Every child is restarted in place
instead of taking the whole Monkey server offline.
"""
...
def main():
 print("GOOL MONKEY starting LIVE TOTAL O/U PROGRUZ pipeline",flush=True)
 ...
 print("GOOL MONKEY ONLINE live_total_ou=on proguz=on bestbet=on sqlite=aux",flush=True)
 stopping=False
 def sig(*_):
