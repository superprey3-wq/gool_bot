 ...
 try:
  ...
  for offer in data.get("betOffers") or []:
   ...
   for outcome in offer.get("outcomes") or []:
    ...
    label=str(outcome.get("label") or "").lower();otype=str(outcome.get("type") or "");side="OVER" if ("over" in label or otype=="OT_OVER") else "UNDER" if ("under" in label or otype=="OT_UNDER") else ""
  ...
  return out
 except Exception as exc:node.LOG.info("KAMBI_TOTAL_FAIL event=%s %s",event["event_id"],type(exc).__name__);return []
